...
- **`root` 必须真的对得上 agent 看到的路径。** 路径比对时，相对路径是相对 `root` 解析的（`root` 默认为 DSH 进程的 `cwd`）。如果 DSH 从一个与工作区无关的目录启动，那么 agent 用相对路径（`contract/contract.judgment.md`）触碰判据时会被解析到工作区之外，插件**不会拦截**。两种稳妥做法：把 `DSH_CLAIM_CHECK_ROOT` 设成工作区绝对路径**并且**要求 agent 用绝对路径；或者直接从工作区目录启动 DSH。**这一条没做对，插件会静默失效**——所以它是本插件最需要留意的地方。
